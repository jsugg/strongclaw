...
def test_workflow_dispatch_supports_cache_benchmarks() -> None:
    ...
    assert dispatch["docker_pull_parallelism"]["default"] == "2"
    assert dispatch["docker_pull_max_attempts"]["default"] == "3"
    ...
    assert dispatch["docker_pull_max_attempts"]["type"] == "string"
...
def test_workflow_dispatch_concurrency_keeps_distinct_benchmarks_alive() -> None:
    ...
    assert "inputs.docker_pull_max_attempts" in workflow_text
...
def test_macos_job_runs_the_full_flow_for_all_events() -> None:
    ...
    assert "macOS Fresh Host Sidecars" in macos_section
    assert "macOS Fresh Host Browser Lab" in macos_section
    assert 'no_activate_services: "true"' in macos_section
    assert 'no_activate_services: "false"' in macos_section
    ...
    assert "FRESH_HOST_SELECTED_DOCKER_PULL_MAX_ATTEMPTS" in macos_section
...
def test_fresh_host_workflow_writes_summaries_and_uploads_reports() -> None:
    ...
    assert "report_dir_name: macos-sidecars" in workflow_text
    assert "report_dir_name: macos-browser-lab" in workflow_text
    ...
    assert "Images requested" in workflow_text
    assert "Missing before pull" in workflow_text
    assert "Pull attempts" in workflow_text
    assert "Retried images" in workflow_text
    assert "Docker image cache" not in workflow_text
...
def test_fresh_host_workflow_points_tools_at_restored_cache_dirs() -> None:
    ...
    assert "--mount-type=virtiofs" in workflow_text
    assert "Hosted CI uses colima on macos-15-intel" in workflow_text
    assert "FRESH_HOST_MACOS_HOST_PROJECT" not in workflow_text
