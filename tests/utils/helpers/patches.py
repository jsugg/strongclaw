"""Tracked patch lifecycle management integrated with ``TestContext``."""
...
class PatchManager:
    ...
    def patch(self, target: str, **kwargs: Any) -> Any:
        ...
        self._context.register_cleanup(f"patch:{target}", patcher.stop)
    ...
    def patch_object(self, target: object, attribute: str, **kwargs: Any) -> Any:
        ...
        mocked = patcher.start()
        self._context.register_cleanup(
    ...
    def patch_dict(self, target: Mapping[str, Any], values: Mapping[str, Any]) -> None:
        ...
        patcher.start()
        self._context.register_cleanup(f"patch.dict:{id(target):#x}", patcher.stop)
        self._patches.append(patcher)
