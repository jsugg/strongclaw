...
def run_extract(
    state_dir: Path, agents_dir: Path, max_bytes_per_file: int, max_messages_per_agent: int
) -> Dict[str, Any]:
    cursor_path = state_dir / "cursor.json"
    cursor = _load_cursor(cursor_path)
    ...
    pending_batches = sorted(
        {v.get("pendingBatch") for v in files.values() if v.get("pendingBatch")}
    )
...
def main() -> int:
    ...
    sub.add_parser("init", help="Initialize cursor to EOF for all current session files")
