      ...
      async execute(_toolCallId, params) {
        ...
        try {
          ...
          const skillContent = [
            "---",
            `name: ${skillName}`,
            `description: "Operational skill extracted from learning ${learningId}."`,
            "---",
            "",
            `# ${skillTitle}`,
            "",
            ...
            "",
            "## When To Use",
            "- The same issue recurs and the response pattern is stable enough to standardize.",
            "- The source learning includes deterministic preconditions and outcomes.",
            "",
            "## Steps",
            "1. Capture prerequisites, command inputs, and required environment variables.",
            "2. Execute the workflow in deterministic order with explicit command examples.",
            "3. Record expected outputs and failure handling guidance for each command.",
            "4. Keep .learnings status aligned with the promoted skill path.",
            "",
            "## Verification",
            "- Re-run the workflow in a clean workspace and confirm expected outputs.",
            "- Confirm the linked learning entry is marked promoted_to_skill.",
            "",
            "## Source Learning",
