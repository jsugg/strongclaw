...
- the rendered runtime config `~/.config/strongclaw/memory/hypermemory.yaml` on Linux
...
- the source template [platform/configs/memory/hypermemory.yaml](../configs/memory/hypermemory.yaml) renders the default runtime config with `backend.active: qdrant_sparse_dense_hybrid` and `backend.fallback: sqlite_fts`
- the source template [platform/configs/memory/hypermemory.sqlite.yaml](../configs/memory/hypermemory.sqlite.yaml) renders the SQLite-only runtime config
...
That flow renders the default StrongClaw stack with `lossless-claw`, `strongclaw-hypermemory`, `autoRecall: true`, `autoReflect: false`, and a managed runtime config derived from [platform/configs/memory/hypermemory.yaml](../configs/memory/hypermemory.yaml).
...
Plain `uv sync` keeps the primary local rerank path on host/Python combinations with known upstream wheel support. Launch-grade CI coverage currently proves that path on Linux x86_64 with Python 3.12/3.13. Compatibility pins for macOS arm64 and Linux aarch64/arm64 (including 64-bit Raspberry Pi 4/5) remain operator-validated best-effort until they are promoted into CI. Unsupported combinations such as Intel macOS or 32-bit Pi Linux skip the local dependency and use `compatible-http` or fail-open behavior instead of blocking setup.
...
The standalone overlay points the plugin at the rendered SQLite runtime config derived from [platform/configs/memory/hypermemory.sqlite.yaml](../configs/memory/hypermemory.sqlite.yaml) and uses the installed `clawops` command.
...
uv run clawops hypermemory verify --json --config ~/.config/strongclaw/memory/hypermemory.yaml
