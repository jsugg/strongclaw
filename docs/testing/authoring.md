...
Prefer a dedicated subsystem directory inside each lane when one already exists.

## Runtime Boundaries

Use `tests/plugins/infrastructure/` for structural test runtime behavior such as `TestContext`, environment management, patch management, profile registration, and framework-owned pytest hooks.
...
Environment mutation, working-directory changes, and patching should flow through the infrastructure runtime:
