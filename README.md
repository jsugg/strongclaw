...
The shipped hypermemory configs now also enable planner-stage reranking with a local `sentence-transformers` provider first and a `compatible-http` fallback. Launch-grade CI coverage currently validates the local rerank dependency on Linux `x86_64` with Python `3.12`/`3.13`. Additional compatibility pins exist for macOS arm64 and Linux aarch64/arm64 (including 64-bit Raspberry Pi 4/5), but those combinations are best-effort until they are promoted into CI. Unsupported combinations such as Intel macOS or 32-bit Raspberry Pi Linux skip the local dependency and rely on `compatible-http` or fail-open behavior instead of breaking installation. The shipped config defaults `rerank.local.device` to `auto`, which selects `cuda` on supported GPU hosts, `mps` on supported Apple Silicon hosts, and `cpu` otherwise. If auto-selected acceleration fails at runtime, the local reranker falls back to CPU automatically.
